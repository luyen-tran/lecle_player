...
import 'package:lecle_player/src/others/lecle_player_utils.dart';
import 'package:screen_brightness/screen_brightness.dart';
import 'package:youtube_explode_dart/youtube_explode_dart.dart';

import 'widgets/lecle_player_menus.dart';

class LeclePlayerControls extends StatefulWidget {
  const LeclePlayerControls(
      {super.key,
  ...
  final Size viewSize;
  final List<LeclePlayerVideoData> videos;
  ...
  final LeclePlayerVideoData selectedQuality;

  @override
  State<LeclePlayerControls> createState() => _LeclePlayerControlsState();
}

class _LeclePlayerControlsState extends State<LeclePlayerControls> {
  VlcPlayerController get player => widget.player;
  LeclePlayerTranslationOptions get translationOptions =>
  ...
  late LeclePlayerVideoData _currentVideoData;
  ...
  List<LeclePlayerCustomActionButton> _customActionButtons = [];
  ...
  void _changeVideoQuality(LeclePlayerVideoData videoData) async {
    ...
    await player.pause().then((value) async {
      ...
      if (videoData.sourceType == LeclePlayerSourceType.network) {
        await player
            .setMediaFromNetwork(videoData.source,
            autoPlay: true, hwAcc: HwAcc.full)
            .whenComplete(() async {
          if (videoData is LeclePlayerVideoDataYoutube) {
      ...
      } else if (videoData.sourceType == LeclePlayerSourceType.file) {
        await player
          ...
          children: [
            GestureDetector(
              onTap: () {
                ...
                LeclePlayerMenus().showQualityOptions(context,
            ...
            GestureDetector(
              onTap: () {
                Navigator.pop(context);
                LeclePlayerMenus().showPlabackSpeedOptions(context,
                    menuColor: getMenuBackgroundColor(),
                    text:
      ...
      onTap: () {
        if (_subtitleTracks != null) {
          if (_subtitleTracks!.entries.isNotEmpty) {
            Navigator.pop(context);
            LeclePlayerMenus().showSubtitleOptions(context,
                menuColor: getMenuBackgroundColor(),
      ...
      onTap: () {
        if (_audioTracks != null) {
          if (_audioTracks![player.value.activeAudioTrack] != null) {
            Navigator.pop(context);
            LeclePlayerMenus().showAudioOptions(context,
                menuColor: getMenuBackgroundColor(),
...
class _VideoControlsSliderToast extends StatefulWidget {
  ...
  final int type;
  final LeclePlayerToastSliderThemeOption volumeSliderTheme;
  final LeclePlayerToastSliderThemeOption brightnessSliderTheme;
